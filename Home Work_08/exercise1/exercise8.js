...
client.connect(async function (err) {
    db = await client.db('restaurants');
...
app.get('/query2', function (req, res) {
    ...
    collection.find({}).project(proj).limit(10).toArray()
        .then(result => {
            res.json(result);
            res.end();
        })
...
app.get('/query9', function (req, res) {
    ...
    const query = { cuisine: { $ne: 'American ' }, 'grades.score': { $gt: 70 }, 'address.coord.0': { $lt: -65.754168 } };
    collection.find(query).limit(2).toArray().then(result => {
        res.json(result);
        res.end();
    })
        .catch(err => console.log(err));
